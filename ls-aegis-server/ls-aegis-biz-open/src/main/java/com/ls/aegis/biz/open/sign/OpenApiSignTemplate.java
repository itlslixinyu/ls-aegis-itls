...
package com.ls.aegis.biz.open.sign;
...
public class OpenApiSignTemplate extends SaSignTemplate {
    ...
    public String createSign(Map<String, ?> paramMap) {
        ...
        String content = super.joinParamsDictSort(paramMap);
        IGmCrypto gmCrypto = gmCryptoProvider.getIfAvailable();
        if (gmCrypto != null && gmCrypto.isEnabled()) {
            return gmCrypto.sm3Hex(content.getBytes(StandardCharsets.UTF_8));
        }
        return SaSecureUtil.md5(content);
    }
}
