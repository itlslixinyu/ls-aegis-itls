...
package com.ls.aegis.common.config.exception;
...
public class GlobalExceptionHandler {
    ...
    public R handleMailAuthenticationException(MailAuthenticationException e, HttpServletRequest request) {
        log.error("[{}] {}", request.getMethod(), request.getRequestURI(), e);
        return R.fail(String.valueOf(HttpStatus.BAD_REQUEST.value()), "邮件服务器认证失败：请检查邮箱账号/密码（或三方客户端安全密码）及 SMTP 配置");
    }

    @ExceptionHandler(MailException.class)
    public R handleMailException(MailException e, HttpServletRequest request) {
        log.error("[{}] {}", request.getMethod(), request.getRequestURI(), e);
        String msg = e.getMessage();
        return R.fail(String.valueOf(HttpStatus.BAD_REQUEST.value()), "邮件发送失败：%s".formatted(CharSequenceUtil
            .blankToDefault(msg, "请检查邮件服务器配置")));
    }
