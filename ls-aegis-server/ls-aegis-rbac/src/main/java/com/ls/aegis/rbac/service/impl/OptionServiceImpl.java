...
import com.ls.aegis.rbac.config.mail.MailSendHelper;
import com.ls.aegis.rbac.enums.OptionCategoryEnum;
import com.ls.aegis.rbac.enums.PasswordPolicyEnum;
import com.ls.aegis.rbac.mapper.OptionMapper;
import com.ls.aegis.rbac.model.entity.OptionDO;
...
public class OptionServiceImpl implements OptionService {
    ...
    public List<OptionResp> list(OptionQuery query) {
        List<OptionResp> list = BeanUtil.copyToList(baseMapper.selectList(QueryWrapperHelper.build(query)), OptionResp.class);
        for (OptionResp resp : list) {
            if (SENSITIVE_OPTION_CODES.contains(resp.getCode()) && StrUtil.isNotBlank(resp.getValue())) {
                resp.setValue(SENSITIVE_OPTION_MASK);
            }
        }
        return list;
    }
    ...
    public void update(List<OptionReq> options) {
        ...
        for (OptionReq req : options) {
            ...
            // 敏感项未改动时前端回传脱敏占位，保留库中原值
            if (SENSITIVE_OPTION_CODES.contains(req.getCode()) && SENSITIVE_OPTION_MASK.equals(req.getValue())) {
                req.setValue(option.getValue());
            }
    ...
    public void sendTestMail(String to) {
        Map<String, String> mail = this.getByCategory(OptionCategoryEnum.MAIL);
        ValidationUtils.throwIfBlank(MapUtil.getStr(mail, "MAIL_HOST"), "请先配置并保存邮件服务器地址");
        ValidationUtils.throwIfBlank(MapUtil.getStr(mail, "MAIL_USERNAME"), "请先配置并保存邮箱账号");
        ValidationUtils.throwIfBlank(MapUtil.getStr(mail, "MAIL_PASSWORD"), "请先配置并保存邮箱密码");
        ValidationUtils.throwIf(SENSITIVE_OPTION_MASK.equals(MapUtil.getStr(mail, "MAIL_PASSWORD")), "邮箱密码无效，请重新填写真实密码后保存");

        Map<String, String> site = this.getByCategory(OptionCategoryEnum.SITE);
        String siteTitle = StrUtil.blankToDefault(site.get("SITE_TITLE"), "LS・Aegis");
        String subject = "【%s】邮件配置测试".formatted(siteTitle);
        String content = """
            <div style="font-family:sans-serif;line-height:1.7;color:#1d2129">
              <p>您好，</p>
              <p>这是一封来自 <b>%s</b> 的测试邮件。</p>
              <p>若您收到本邮件，说明当前 SMTP 配置可用。</p>
              <p style="color:#86909c;font-size:12px">发件账号：%s<br/>服务器：%s:%s</p>
            </div>
            """.formatted(siteTitle, MapUtil.getStr(mail, "MAIL_USERNAME"), MapUtil.getStr(mail, "MAIL_HOST"), MapUtil
            .getStr(mail, "MAIL_PORT"));
        MailSendHelper.sendHtml(to, subject, content);
    }
