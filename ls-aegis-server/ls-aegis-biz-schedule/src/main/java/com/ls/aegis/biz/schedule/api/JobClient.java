...
import com.ls.aegis.biz.schedule.constant.JobConstants;
import com.ls.aegis.biz.schedule.exception.ScheduleClientException;
import com.ls.aegis.biz.schedule.exception.ScheduleServerException;
import com.ls.aegis.biz.schedule.model.JobPageResult;
...
public class JobClient {
    ...
    private String authenticate() {
        ...
        // 服务端校验：sha256(线上口令摘要)；国密开启时摘要为 SM3，关闭时兼容官方 MD5
        paramMap.put("password", digestPassword(password));
    ...
    /**
     * 登录口令摘要：gm.enable=true 使用 SM3，否则 MD5（与官方控制台一致）。
     * <p>调度中心库中密码须为 {@code sha256(摘要)}，国密环境请执行 ls-aegis-biz-schedule 文档中的口令更新说明。</p>
     */
    static String digestPassword(String rawPassword) {
        IGmCrypto gmCrypto = ExceptionUtils.exToNull(() -> SpringUtil.getBean(IGmCrypto.class));
        if (gmCrypto != null && gmCrypto.isEnabled()) {
            return gmCrypto.sm3Hex(rawPassword);
        }
