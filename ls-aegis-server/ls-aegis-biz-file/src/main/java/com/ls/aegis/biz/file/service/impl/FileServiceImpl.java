...
public class FileServiceImpl extends BaseServiceImpl<FileMapper, FileDO, FileResp, FileResp, FileQuery, FileReq> implements FileService {
    ...
    private String pretreatmentPath(String path) {
        ...
        String normalized = StrUtil.blankToDefault(path, StringConstants.EMPTY).replace('\\', '/');
        CheckUtils.throwIf(normalized.contains(".."), "非法路径");
        return StrUtil.appendIfMissing(StrUtil.removePrefix(normalized, StringConstants.SLASH), StringConstants.SLASH);
    }
