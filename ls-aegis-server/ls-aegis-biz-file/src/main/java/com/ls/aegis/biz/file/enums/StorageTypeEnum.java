...
package com.ls.aegis.biz.file.enums;
...
import com.ls.aegis.biz.file.model.req.StorageReq;
import com.ls.aegis.common.validation.ValidationGroup;
    ...
    LOCAL(1, "本地存储") {
        ...
        public void validate(StorageReq req) {
            ...
            ValidationUtils.throwIf(StrUtil.isNotBlank(req.getDomain()) && !URLUtils.isHttpUrl(req
                .getDomain()), "访问路径格式不正确");
            // 禁止本地存储路径穿越（如 ../）
            String bucket = StrUtil.blankToDefault(req.getBucketName(), StringConstants.EMPTY).replace('\\', '/');
            ValidationUtils.throwIf(bucket.contains(".."), "本地存储路径不能包含 ..");
            String recycle = StrUtil.blankToDefault(req.getRecycleBinPath(), StringConstants.EMPTY).replace('\\', '/');
            ValidationUtils.throwIf(recycle.contains(".."), "回收站路径不能包含 ..");
        }
